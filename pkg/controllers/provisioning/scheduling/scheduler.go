...
func NewScheduler(
	...
	nodeToNodePool := lo.SliceToMap(stateNodes, func(n *state.StateNode) (string, *v1.NodePool) {
		return n.Name(), npByName[n.Node.Labels[v1.NodePoolLabelKey]]
	})
	s.calculateExistingNodeClaims(ctx, stateNodes, daemonSetPods, nodeToNodePool)
	return s
}
...
func (s *Scheduler) calculateExistingNodeClaims(ctx context.Context, stateNodes []*state.StateNode, daemonSetPods []*corev1.Pod, nodePoolMap map[string]*v1.NodePool) {
	// create our existing nodes
	for _, node := range stateNodes {
		taints := node.Taints()
		daemons := s.getCompatibleDaemonPods(ctx, node, taints, daemonSetPods)
		s.existingNodes = append(s.existingNodes, NewExistingNode(node, s.topology, taints, resources.RequestsForPods(daemons...), disruption.IsUnderConsolidateAfter(nodePoolMap[node.Name()], node.NodeClaim, s.clock)))
